...
def read_document_data(db: Session) -> List[Tuple[str, str, StatusEnum, str]]:
    ...
    """
    try:
        results = db.query(Document.path, Document.file, Document.status, Document.timestamp).all()
...
def main():
    ...
    try:
        ...
        for path, file, status, timestamp in data:
            print(f"Path: {path}, File: {file}, Status: {status.value}, Timestamp: {timestamp}")
    finally:
        db.close()
